...
fixed_rates = {
    ...
    "BIOMASS":0.281979407  # Fix biomass reaction rate
}
...
in_vivo_yield = 0.281979407
in_vivo_sd = 0.01400768
biomass_weight = 1
...
print(f"Biomass bounds within 10% of the maximum: [{biomass_reaction.lower_bound}, {biomass_reaction.upper_bound}]")
...
""" # List of reactions for which to perform FVA
reactions_of_interest = [
    "EXCH_etoh_e",  # Ethanol exchange
    "EXCH_ac_e",    # Acetate exchange
    "EXCH_for_e",   # Formate exchange
    "EXCH_lac__L_e",  # Lactate exchange
    "EXCH_pyr_e",   # Pyruvate exchange
    "EXCH_mal__L_e"  # Malate exchange
]

# Run FVA for the specified reactions
fva_results = flux_variability_analysis(model, reaction_list=reactions_of_interest, fraction_of_optimum=1.0)

# Print results for each reaction
for reaction_id in reactions_of_interest:
    reaction = model.reactions.get_by_id(reaction_id)
    ...
    print(
        f"{reaction.id}\t{reaction.lower_bound:.8f}\t{min_flux:.8f}\t{max_flux:.8f}\t{reaction.upper_bound:.8f}"
    )
